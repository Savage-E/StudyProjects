namespace BalancedBinaryTree
{
    public class BinaryTree<T> where T : System.IComparable<T>
    {
        ...
        private int Bfactor(Node<T> node)//Calculating the difference height of left and right subtrees.
        {
            return GetHeight(node.left) - GetHeight(node.right);
        }

        private void CorrectHeight(Node<T> node)//Corrects the height of node after transformations.
        {
        ...
        private Node<T> InsertNode(Node<T> current, T data)
        {
            ...
            if (data.CompareTo(current.data) > 0)
                ...
                return current;//Equal values are not allowed.
        ...
        private Node<T> RotateLL(Node<T> parent)//Left Left Case( right turn).
        {
        ...
        private Node<T> RotateRL(Node<T> parent)//Right Left case(Left turn then right).
        {
        ...
        private Node<T> RotateLR(Node<T> parent)//Left Right case(Right turn then left).
        {
            Node<T> pivot = parent.left;
        ...
        private Node<T> RotateRR(Node<T> parent)//Right Right case(Turn left).
        {
            Node<T> pivot = parent.right;
        ...
        private Node<T> BalanceTree(Node<T> node)//Balancing the tree.
        {
